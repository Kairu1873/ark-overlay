...
- **テイム計算**：レベルを入れると、食べる物ごとに **必要な個数・所要時間・必要な麻酔**（ナルコベリー／麻酔薬／バイオトキシン）が出る。平和テイムの生物は給餌間隔と合計時間になる
...
テイムの計算式は英語Wikiの `Module:TamingTable` と同じものを使っており、Wiki が公開している
テイム表と値が一致することを `npm test` で固定している。テイム効率（TE）とボーナスレベルは、
裏の取れる計算式が無いため出していない。
...
**同じ項目が両方にある場合は日本語Wikiを採る。** ASA では英語Wikiの更新が追いついておらず、
「英語Wikiに無いが日本語Wikiにはある」が頻発するためである。英語Wikiは補う側として使う。

| | 供給するもの |
|---|---|
| 日本語Wiki（wikiwiki.jp/arksa） | 繁殖時間・ステータス・成長率・出現マップ・テイム方法・餌の優先順位 |
| 英語Wiki（ark.wiki.gg） | テイム係数・交配クールダウン・アイテム・レシピと、日本語Wikiに無いものの穴埋め |
