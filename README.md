...
あれば裏でダウンロードする。×で閉じてトレイにいる間にもう一度起動したとき（ショートカットや
スタートメニューから開き直したとき）も見に行く。落とし終えると画面の上部とトレイに「更新して再起動」が出る。
...
- 交配クールダウンは「最短 〜 最長」の幅で表示し、タイマーは **確実に明けている最長側** で鳴らす（実測値を入れてあればその値）
...
- 2文字以上で使える文字列が無いときは、1文字まで下げて探す（`Stone` と `Metal` から `Obsidian` を外すと `"t"` になる）
